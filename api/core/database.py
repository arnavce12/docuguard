...
def save_scan(
    ...
    if health_score:
        data["health_score_overall"] = health_score.get("overall")
        data["health_score_grade"] = health_score.get("grade")
        data["health_score_axes"] = health_score.get("axes")
        data["health_score_flags"] = health_score.get("flags")
...
def update_scan_name(scan_id: str, scan_type: str, user_id: str, new_name: str) -> bool:
    """Update the display name of a scan (document_label for fraud, common_name for kyd)."""
    table = "scans" if scan_type == "fraud" else "kyd_analyses"
    column = "document_label" if scan_type == "fraud" else "common_name"
    
    response = (
        supabase.table(table)
        .update({column: new_name})
        .eq("id", scan_id)
        .eq("user_id", user_id)
        .execute()
    )
...
def save_kyd_scan(
    ...
    key_fields_present: list,
    data_categories: list,
    notes: str,
    storage_path: str = None,
    user_id: str = None
) -> dict:
    """Save a KYD scan result to the database."""
    data = {
    ...
    response = supabase.table("kyd_analyses").insert(data).execute()
    return response.data[0] if response.data else None


def get_user_kyd_scans(user_id: str) -> list:
    """Fetch all KYD scans for a specific user, newest first."""
    response = (
        supabase.table("kyd_analyses")
        .select("*")
        .eq("user_id", user_id)
        .order("created_at", desc=True)
        .execute()
...
def delete_user_kyd_scan(scan_id: str, user_id: str) -> bool:
    """Delete a KYD scan only if it belongs to the requesting user."""
    response = (
        supabase.table("kyd_analyses")
        .delete()
        .eq("id", scan_id)
